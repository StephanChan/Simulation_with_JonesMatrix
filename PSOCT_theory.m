...
J_samp_arm=subs(Jones,x,d/180*pi);
J_samp_arm=subs(J_samp_arm,t,0);
J_QWP_samp=subs(Jones,x,pi/2);
J_QWP_samp=subs(J_QWP_samp,t,pi/4);

%% reference arm
E_ref=simplify(J_QWP_ref*exp(-j*2*k*zr-j*pi)*normal_glass*J_QWP_ref*[1;0]/2);

%% sample arm
%signal_samp=simplify(J_QWP2*Jones*Jones*J_QWP2*[1;0]*sqrt(R)/2);
E_samp=simplify(J_QWP_samp*J_samp_arm*J_samp_arm*J_QWP_samp*[1;0]/2);
